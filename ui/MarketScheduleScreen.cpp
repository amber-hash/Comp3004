...
void MarketScheduleScreen::populateTable() {
    ...
    for (int i = 0; i < dates.size(); ++i) {
        ...
        // Col 2: Status
        QString statusText  = (avail > 0) ? "Available" : "Waitlist Only";
        QColor  statusColor = (avail > 0) ? QColor("#3D6B4F") : QColor("#C47C20");
...
void MarketScheduleScreen::updateActionButtons() {
    ...
    m_bookBtn->setEnabled(isCompliant && avail > 0 && !hasBooking && !onWaitlist);
    ...
    QString status;
    if (!isCompliant)    status = "⚠  Complete your compliance documents to enable booking.";
    else if (hasBooking) status = "✅  You have a booking for this date. You may cancel it.";
    else if (onWaitlist) {
        int pos = m_manager->getWaitlistPosition(m_vendor, md);
        status = QString("⏳  You are #%1 on the waitlist for this date.").arg(pos);
    }
    else if (avail > 0)  status = QString("📋  %1 stall(s) available. Click \"Book Stall\" to reserve.").arg(avail);
    else                 status = "⏳  No stalls available. You may join the waitlist.";
    ...
    if (confirm.exec() == QMessageBox::Yes) {
        ...
        if (m_manager->bookStall(m_vendor, md)) {
            ...
            QMessageBox::warning(this, "Booking Failed",
                "Unable to complete booking. The stall may no longer be available.");
        }
        refresh();
    }
}
